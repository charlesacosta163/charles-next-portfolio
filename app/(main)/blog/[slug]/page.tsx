...
export default async function Page({params}: {params: {slug: string}}) {
  ...
  return (
    <div className="max-w-[800px] w-full bg-white dark:bg-base2-dark p-8 sm:px-4 rounded-[20px] sm:text-sm">
      ...
      <h1 className="text-[2em] sm:[1.5rem] font-[600] tracking-tight sm:leading-8">{article.fields.title}</h1>
      ...
      <div className="text-xs font-[500]">Published on {formatDate(article.sys.createdAt)}</div>
    </div>
  );
}
