...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description =
    "Explore Charles Acosta's latest blog posts on web development, technology, and more.";
...
export default async function Page() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-[2rem] tracking-tight font-semibold">Blogs 📝</h1>
          ...
          ? entries.items.map((entry: any) => {
              return (
                <Link
                  ...
                  key={entry.sys.id}
                  className="flex sm:flex-col-reverse sm:max-w-[400px] hover:-translate-y-1 transition-all sm:w-full gap-4 justify-between p-8 sm:p-4 cursor-pointer bg-white dark:bg-base2-dark rounded-[20px] shadow-md"
                >
